...
/* ─────────────────────────────────────────────────────────────────────────────
   FIX 3: Sidebar toggle button — always visible, even when sidebar is closed
   ───────────────────────────────────────────────────────────────────────────── */
...
[data-testid="collapsedControl"] {
    ...
    background: rgba(2,8,24,0.9) !important;
    border: 1px solid rgba(245,197,24,0.4) !important;
    border-radius: 50% !important;
    color: #F5C518 !important;
    box-shadow: 0 0 14px rgba(245,197,24,0.5), 0 0 30px rgba(0,212,255,0.2) !important;
    transition: all 0.3s ease !important;
}
button[data-testid="collapsedControl"]:hover,
button[aria-label="Open sidebar"]:hover {
    box-shadow: 0 0 24px rgba(245,197,24,0.8), 0 0 50px rgba(0,212,255,0.4) !important;
...
/* ─────────────────────────────────────────────────────────────────────────────
   FIX 2: Neon glow buttons — using attribute/class selectors that actually work
   in Streamlit's rendered DOM instead of fragile nth-child positional selectors
   ───────────────────────────────────────────────────────────────────────────── */
...
.stButton > button {
    ...
    box-shadow: 0 0 14px rgba(0,212,255,0.35), inset 0 0 12px rgba(0,212,255,0.05) !important;
}
.stButton > button:hover {
    background: rgba(0,212,255,0.08) !important;
    box-shadow: 0 0 30px rgba(0,212,255,0.75), 0 0 60px rgba(0,212,255,0.25), inset 0 0 20px rgba(0,212,255,0.08) !important;
    transform: translateY(-3px) scale(1.03) !important;
    color: #fff !important;
    text-shadow: 0 0 16px rgba(0,212,255,1) !important;
}
.stButton > button:focus {
    box-shadow: 0 0 0 2px rgba(0,212,255,0.5), 0 0 20px rgba(0,212,255,0.4) !important;
    outline: none !important;
}

/* Process Documents button — Gold neon (sidebar, first action button) */
div[data-testid="stSidebar"] .stButton:first-of-type > button,
div[data-testid="stSidebar"] .stButton > button[kind="primary"] {
    border-color: #F5C518 !important;
    color: #F5C518 !important;
    text-shadow: 0 0 8px rgba(245,197,24,0.9) !important;
    box-shadow: 0 0 14px rgba(245,197,24,0.4), 0 0 4px rgba(255,45,120,0.15), inset 0 0 12px rgba(245,197,24,0.05) !important;
}
div[data-testid="stSidebar"] .stButton:first-of-type > button:hover {
    background: rgba(245,197,24,0.08) !important;
    box-shadow: 0 0 35px rgba(245,197,24,0.8), 0 0 70px rgba(245,197,24,0.3), 0 0 12px rgba(255,45,120,0.4) !important;
    color: #fff !important;
    text-shadow: 0 0 18px rgba(245,197,24,1) !important;
    transform: translateY(-3px) scale(1.03) !important;
}

/* Sidebar NAV buttons — individual neon colors via key override using data-testid spans */
/* Q&A Chat → Cyan */
div[data-testid="stSidebar"] .stButton > button[aria-label*="Q&A"],
div[data-testid="stSidebar"] .stButton > button:has(+ *) {
    border-color: #00D4FF !important;
    color: #00D4FF !important;
}

/* Summary → Purple */
div[data-testid="stSidebar"] .stButton > button[aria-label*="Summary"] {
    border-color: #BF5FFF !important;
    color: #BF5FFF !important;
    text-shadow: 0 0 8px rgba(191,95,255,0.8) !important;
    box-shadow: 0 0 14px rgba(191,95,255,0.3) !important;
}

/* Quiz → Green */
div[data-testid="stSidebar"] .stButton > button[aria-label*="Quiz"] {
...
/* Compare → Orange */
div[data-testid="stSidebar"] .stButton > button[aria-label*="Compare"] {
...
/* Hover upgrades for sidebar nav buttons */
div[data-testid="stSidebar"] .stButton > button:hover {
    transform: translateY(-3px) scale(1.03) !important;
    color: #fff !important;
}

/* Main area buttons — keep cyan glow but slightly more intense */
section.main .stButton > button {
    border-color: #00D4FF !important;
    color: #00D4FF !important;
    box-shadow: 0 0 16px rgba(0,212,255,0.4), inset 0 0 16px rgba(0,212,255,0.06) !important;
}
section.main .stButton > button:hover {
    box-shadow: 0 0 35px rgba(0,212,255,0.75), 0 0 70px rgba(0,212,255,0.25), 0 0 10px rgba(191,95,255,0.4) !important;
}

/* Topic / popular subject chip buttons — compact gold */
section.main div[data-testid="stHorizontalBlock"] .stButton > button {
    font-size: 0.65rem !important;
    padding: 0.35rem 0.7rem !important;
    border-color: rgba(245,197,24,0.6) !important;
    color: #F5C518 !important;
    text-shadow: 0 0 6px rgba(245,197,24,0.7) !important;
    box-shadow: 0 0 10px rgba(245,197,24,0.25) !important;
}
section.main div[data-testid="stHorizontalBlock"] .stButton > button:hover {
    background: rgba(245,197,24,0.08) !important;
    box-shadow: 0 0 22px rgba(245,197,24,0.65) !important;
    color: #fff !important;
}
...
(function() {
    ...
    /* ── FIX 3 (JS layer): keep sidebar toggle visible after collapse ── */
    ...
    const observer = new MutationObserver(ensureSidebarToggleVisible);
        ...
        if st.button("🔍 Compare Documents", use_container_width=True):
            ...
            if not raw_a or not raw_b:
                ...
                with st.spinner("🔍 Comparing documents..."):
                    try:
                        # ── FIX 1: Pass a comparison topic derived from both filenames ──
                        # compare_documents() requires a 'topic' argument; we build one
                        # from the document names so it always has a meaningful value.
                        comparison_topic = f"comparison between '{doc_a}' and '{doc_b}'"
                        comparison = compare_documents(raw_a, doc_a, raw_b, doc_b, comparison_topic)
